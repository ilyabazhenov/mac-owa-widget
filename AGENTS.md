...
- Полный цикл включает:
  ...
  2. Обновление `RELEASE_NOTES.md` под эту версию.
     - Для каждой версии обязателен формат:
       - `## vX.Y.Z - YYYY-MM-DD`
       - `### RU` + `### EN`
       - в обоих языках обязательны подразделы про изменения и установку.
     - В `RELEASE_NOTES.md` обязательно добавлять инструкцию по установке на русском и английском языках.
     - В инструкции по установке в обеих языковых секциях обязательно упоминать и приводить команду: `xattr -dr com.apple.quarantine /Applications/OWAWidget.app`.
